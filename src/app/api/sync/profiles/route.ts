...
export async function POST(request: Request) {
  return tryRoute(async () => {
    const user = await getCurrentUser();
    if (!user) return NextResponse.json({ ok: false, error: "请先登录后同步" }, { status: 401 });
    ...
    const localProfileId = String(body.localProfileId || "").trim().slice(0, 80);
    ...
    const calendarType = (String(input.calendarType) === "lunar" ? "lunar" : "solar") as CalendarType;
    const genderInput = String(input.gender || "");
    const gender = (["male", "female", "other"].includes(genderInput) ? genderInput : "other") as Gender;
    const timeUnknown = (input as Record<string, unknown>).birthTimeKnown === false || Boolean(input.timeUnknown);
    const isLeapMonth = calendarType === "lunar" && Boolean(input.isLeapMonth);
    if (!localProfileId || !birthDate) {
      return NextResponse.json({ ok: false, error: "本地档案缺少标识或出生日期" }, { status: 400 });
    }
    ...
    if (mappedProfile) {
      if (profileSignature(mappedProfile) !== profileSignature(draft)) {
        if (resolution === "keep-cloud") {
          ...
          return NextResponse.json({ ok: true, data: { profile: mappedProfile, syncState: synced, resolution } });
        }
        if (resolution === "keep-local") {
          ...
          const synced = await saveSyncState(user.id, localProfileId, mappedProfile.id);
          return NextResponse.json({ ok: true, data: { profile: replacement, syncState: synced, resolution } });
        }
        ...
        return NextResponse.json({ ok: false, error: conflict.error, conflict: { local: input, cloud: mappedProfile }, syncState: conflict }, { status: 409 });
      }
      const synced = await saveSyncState(user.id, localProfileId, mappedProfile.id);
      return NextResponse.json({ ok: true, data: { profile: mappedProfile, syncState: synced, reused: true } });
    }
    ...
    if (duplicate) {
      ...
      return NextResponse.json({ ok: true, data: { profile: duplicate, syncState: synced, reused: true } });
    }
    ...
    return NextResponse.json({ ok: true, data: { profile, syncState: synced, reused: false } }, { status: 201 });
  });
}
